...
module.exports = async (req, res) => {
  // هيدرز الـ CORS للسماح بالاتصال من البوت أو الموقع
  ...
  try {
    // استقبال المتغير في فيرسل يكون عبر req.query مباشرة وبمنتهى السهولة
    ...
    if (!query) {
      return res.status(400).json({
        ...
        error: 'يرجى كتابة نص البحث في معامل q'
      });
    }
    ...
    if (!apiRes.data || !apiRes.data.songs || !Array.isArray(apiRes.data.songs)) {
      return res.status(502).json({
        success: false,
        creator: "By Arab Top Dev",
        error: 'لم يتم العثور على نتائج من المصدر.'
      });
    }
    ...
    return res.status(200).json({
      success: true,
      creator: "By Arab Top Dev",
      search_engine: "Spotify Vercel API",
